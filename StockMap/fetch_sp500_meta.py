...
LIST_FILE  = BASE / "sp500_list.json"        # 100 只代码的静态 JSON
META_FILE  = BASE / "sp500_meta.json"        # 累积写入的元数据
...
for idx in range(start, len(tickers)):
    ...
    try:
        ...
        print(f"[{idx+1:>3}/500] {t:<6} cap={cap_str:<12}  wso={wso_str}")
    except Exception as e:
        print(f"[{idx+1:>3}/500] {t}  ! error: {e}")
